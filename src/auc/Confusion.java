...
public class Confusion extends Vector
{
        ...
        throws NumberFormatException
    {
    	...
    	System.out.println("wzy:\t"+d+"\t"+d1+"\t"+totPos+"\t"+totNeg
    			);
    ...
    /**
     * This the function calculates the PR-AUC
     * @author Zhuoyu Wei
     * @version 2.0 print the result to the log file.
     * @param d I don't know....
     * @param ps a print stream to replace the system.out
     */
    public double calculateAUCPR(double d,PrintStream ps)
    {
        if(AUCCalculator.DEBUG)
            ps.println("--- Calculating AUC-PR ---");
        if(d < 0.0D || d > 1.0D)
        {
            ps.println("ERROR: invalid minRecall, must be between 0 and 1 - returning 0");
            return 0.0D;
        }
        if(size() == 0)
        {
            ps.println("ERROR: No data to calculate....");
            return 0.0D;
        }
        double d1 = d * totPos;
        int i = 0;
        ...
        {
            ps.println("ERROR: minRecall out of bounds - exiting...");
            System.exit(-1);
        }
        double d2 = (pnpoint.getPos() - d1) / totPos;
        double d3 = pnpoint.getPos() / (pnpoint.getPos() + pnpoint.getNeg());
        ...
        ps.println((new StringBuilder()).append("Area Under the Curve for Precision - Recall is ").append(d4).toString());
        return d4;
    }
    ...
    /**
     * This the function calculates the ROC-AUC
     * @author Zhuoyu Wei
     * @version 2.0 print the result to the log file.
     * @param ps a print stream to replace the system.out
     */    
    public double calculateAUCROC(PrintStream ps)
    {
        if(AUCCalculator.DEBUG)
            ps.println("--- Calculating AUC-ROC ---");
        if(size() == 0)
        {
            ps.println("ERROR: No data to calculate....");
            return 0.0D;
        ...
        d2 = 1.0D - d2;
        ps.println((new StringBuilder()).append("Area Under the Curve for ROC is ").append(d2).toString());
        return d2;
    }
