...
public class RandomWalkThread implements Callable{
	...
	{
		StatePOG nextState=null;
		Integer randIndex=Math.abs(rand.nextInt())%nextOperation[4]+1;
		int i;
		for(i=0;i<5;i++)
		{
			if(randIndex<=nextOperation[i])
	...
	private boolean RandomWalkOrdinaryWithCheck(StatePOG nowState,double restartProbability)
	{
		...
		//remove restart
		//nextOperation[4]=(int)(nextOperation[3]*(1.+restartProbability));
		nextOperation[4]=(int)(nextOperation[3]*1.4);
		...
		for(int i=0;i<maxround;i++)
		{
			try{
				...
				if(trainortestflag)
					...
					flag=RandomWalkWithProbs(obj_nowState,GlobalStaticData.restartProbability);
	...
	public boolean RandomWalkWithProbs(StatePOG nowState,double restartProbability)
	{
		double randvalue=rand.nextDouble();
		if(randvalue<0.35)
		{
		...
		if(nowState.state.size()>=GlobalStaticData.MaxLength||(randvalue<0.5&&nowState.state.size()>1))
		{
			randvalue=rand.nextDouble();
			/*if(nowState.state.get(nowState.state.size()-1).equals(startPoint))
			{
				nowState=removeAhead(nowState);
			}
			else if(nowState.state.get(0).equals(startPoint))
			{
				nowState=removeBack(nowState);					
			}
		...
		{
			List<Integer> candEntity=new ArrayList<Integer>();
			List<Double> candProb=new ArrayList<Double>();

			double curve=0.;
			
			Integer entity0=nowState.state.get(0);
			HyperNode hnode0=GlobalStaticData.entityToNode.get(entity0);
			for(int i=0;i<hnode0.neighbourList.size();i++)
			{
				Integer entity2=hnode0.neighbourList.get(i);
				Double prob=GlobalStaticData.candidateToProb.get(entity2);
				if(prob==null)
					prob=1.;
				candEntity.add(entity2);
				curve+=prob;
				//curve+=1.0;
				candProb.add(curve);
			}
			double leftcurve=curve;
			//if(nowState.state.size()>1)
			if(true)
			{
				Integer entity1=nowState.state.get(nowState.state.size()-1);
				HyperNode hnode1=GlobalStaticData.entityToNode.get(entity1);
				for(int i=0;i<hnode1.neighbourList.size();i++)
				{
					Integer entity2=hnode1.neighbourList.get(i);
					Double prob=GlobalStaticData.candidateToProb.get(entity2);
					if(prob==null)
						prob=1.;
					candEntity.add(entity2);
			...
			//pslog.println(candProb.size()+"\t"+curve+"\t"+leftcurve);
			for(int i=0;i<candProb.size();i++)
			{
				candProb.set(i, candProb.get(i)/curve);
				//pslog.print(candProb.get(i)+"\t");
			}

			leftcurve/=curve;
			//pslog.println("\n"+(curve/curve)+"\t"+leftcurve);		
			...
			while(true)
			{
				randvalue=rand.nextDouble();
				int i;
				for(i=0;i<candProb.size();i++)
				{
					if(randvalue<candProb.get(i))
						break;
				}
				Integer nextEntity=candEntity.get(i);
				if(randvalue<leftcurve)
				{
					nextState=pushAhead(nowState,nextEntity);
				}
				else
				{
					nextState=pushBack(nowState,nextEntity);
				}
				//pslog.println(randvalue+"\t"+i);
				if(CheckDuplicate(nextState))
				{
					break;
				}
			}
		...
		obj_nowState=nowState;
		saveStateToList(nowState);
